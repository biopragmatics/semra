...
- When you run ``python3 -m semra`` python will execute``__main__.py`` as a script. That
  means there won't be any ``semra.__main__`` in ``sys.modules``.
- When you import __main__ it will get executed again (as a module) because there's no
  ``semra.__main__`` in ``sys.modules``.
